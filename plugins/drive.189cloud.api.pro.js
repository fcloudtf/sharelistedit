const name = '189CloudAPIPRO'

const version = '1.0'

const protocols = ['ctcpro']

const defaultProtocol = 'ctcpro'
...
//    this.OAUTH2_TOKEN_URL = "https://cloud.189.cn/open/oauth2/accessToken.action"
    this.OAUTH2_AUTH_BASE_URL = "https://cloud189.cf/open/oauth2/authorize.action"
    this.OAUTH2_TOKEN_URL = "https://cloud189.cf/open/oauth2/accessToken.action"
...
const install = async (app_id, app_secret, redirect_uri) => {
  return `
    <div class="auth">
      <h3>天翼云 挂载向导</h3>
      <p style="font-size:12px;">下面参数任意填写即可</p>
      <div>
        <form class="form-horizontal" method="post">
          <input type="hidden" name="act" value="install" />
          <input type="hidden" name="redirect_uri" value="${redirect_uri}" />
          <div class="form-group"><input class="sl-input" type="text" name="app_secret" value="1" placeholder="应用机密 / app_secret" /></div>
          <div class="form-group"><input class="sl-input" type="text" name="app_id" value="1" placeholder="应用ID / app_id" /></div>
...
//        url:`https://api.cloud.189.cn${url}`,
  ...
  return { name, label:'天翼云 API PRO', version, drive: { protocols, folder, file , createReadStream  } }
}